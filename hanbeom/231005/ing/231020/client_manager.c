...
//		
		...
		endTime.tm_year = client[i].end_year - 1900;
		endTime.tm_mon = client[i].end_month - 1;
		endTime.tm_mday = client[i].end_day;
		endTime.tm_hour = client[i].end_hour;
		endTime.tm_min = client[i].end_min;
		endTime.tm_sec = 0;
		endTime.tm_isdst = -1;
		...
		double diffInSeconds = difftime(endTimestamp, currentTime);	// - is not
		//printf("diffInSeconds is %f\n", diffInSeconds); //test
...
//
		...
		else if(strcmp(client[i].name, name_func2) == 0 && client[i].delete != 1 && diffInSeconds < 0)	//3
		{
			...
			client[i].months = months_func2;
			client[i].start_year = timeInfo->tm_year + 1900;
			client[i].start_month = timeInfo->tm_mon +1;
			client[i].start_day = timeInfo->tm_mday;
			client[i].start_hour = timeInfo->tm_hour;
			client[i].start_min = timeInfo->tm_min;
